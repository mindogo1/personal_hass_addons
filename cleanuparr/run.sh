...
APPSETTINGS_CONTENT=$(cat <<EOF
{
  ...
  "Cleanuparr": {
    ...
    "DryRun": $DRY_RUN
    // Add other Cleanuparr settings here as needed, exposing them in config.yaml
    // Example: "ContentBlocker": { "Enabled": true, "IntervalMinutes": 5 }
  }
}
EOF
)
...
# Cleanuparr expects this file in its working directory (which is /app due to WORKDIR in Dockerfile)
